...
const compact=(items:any[],maxChars=4000)=>{const out:any[]=[];let used=0;for(const item of items){const x={id:item.id,title:item.title,summary:item.summary??item.content,confidence:item.confidence};const s=JSON.stringify(x);if(used+s.length>maxChars)break;out.push(x);used+=s.length;}return{items:out,budget:{maxChars,returnedItems:out.length,returnedChars:used,truncated:out.length<items.length}};};
...
export async function handleMcpRpc(store:IntelligenceStore,msg:Rpc,databaseUrl?:string){
 ...
 if(name==='intelligence_search'){const p=RetrievalInput.safeParse({...input,limit:Math.min(input.limit??2,5)});if(!p.success)throw new Error('INVALID_RETRIEVAL');const found=await store.retrieve(p.data);return ok(msg.id,compact(found,input.maxChars??4000));}
